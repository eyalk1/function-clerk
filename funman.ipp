...
funman<f, Args...>::funman(f func):
    func_m(func){
static constexpr auto all_types_are_different = ((count_type<Args, Args...>() == 1) && ...);
static assert(std::is_invocable_v<f, Args...>, "The combination of function and the list of types given cannot be called");
static_assert(all_types_are_different, "Not all the types that were given to the funman are different");
}
...
void funman<f, Args...>::operator()(arg_t&&... new_arg){
    ...
    static constexpr auto all_types_are_different = ((count_type<Args,Args...>() == 1) && ...);
    static_assert(all, "not all the types that were giner are part of the function parameter list");
    static_assert(all_types_are_different, "Not all the types that were given are different");

    std::tie(std::get<get_t_index<arg_t, Args...>()>(arguments)...) = std::tie((new_arg)...);
}
...
void funman<f, Args...>::unValid(){
    ...
    static_assert(all, "not all the types that were given are part of the function parameter list");
...
auto funman<f, Args...>::call_impl(std::index_sequence<I...>)->funman<f, Args...>::R{
...
bool funman<f, Args...>::areAllValid_impl(std::index_sequence<I...>){
    return (isValid<typename std::tuple_element_t<I, args_t>::value_type>() || .. || 0);
}
